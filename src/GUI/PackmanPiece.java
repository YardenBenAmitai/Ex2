...
public class PackmanPiece extends JPanel implements GamePiece {
	
	private static final long serialVersionUID = 1L;
	private Map map;
	public List<FruitPiece> fp;
	private ImageIcon iip=new ImageIcon("img/Pacman.png");
	private Timer timer;
	...
	public PackmanPiece(ArrayList<Fruit> f, Map m){
		map=m;
		ip=map.CoordsToPixels(f.get(0).getData().get_Orientation());
		currX=ip.ix();
		currY=ip.iy();
		f.remove(0);
		fp=new ArrayList<FruitPiece>();
		Fruit fruit;
		Iterator<Fruit> it=f.iterator();
		while (it.hasNext()){
			fruit=it.next();
			fp.add(new FruitPiece(m.CoordsToPixels(fruit.getData().get_Orientation())));
		}
		if(!fp.isEmpty()){

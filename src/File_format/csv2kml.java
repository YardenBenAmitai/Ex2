...
/**
 * Description:
 * csv2kml class is mainly a converting class.
 * the class has an empty constructor,
 * and two methods converting csv files into kml while constructing points and layers.
 * 
...
public class csv2kml extends GPS_layer{
	...
	/**
	 * Description:
	 * the method receives GPS_layer, csv file path and kml file path.
	 * the method follows the csv file path,
	 * reads from it and converts it into a kml file, located at the kml file path.
	 * the method also update the given layer according to the csv file.
	 * 
	 * @param layer
	 * @param csvFileName
	 * @param kmlFileName
	...
	/**
	 * Description:
	 * the method receives a GPS_layer, StringBuilder and a csv file.
	 * the method reads from the given csv file and appends the kml-relevant information into the StringBuilder
	 * while creating GPS_Points accordingly and adding them to the given layer.
	 * 
	 * @param layer
	 * @param content
	 * @param csvFileName
		...
		try{
			...
			while ((curLine = csvReader.readLine()) != null) {
				...
				String kmlelement ="<Placemark>\n" +
	                    "<name>"+s[1]+"</name>\n" +
						"<styleUrl>"+layer.get_data().get_colour()+"</styleUrl>\n"+
